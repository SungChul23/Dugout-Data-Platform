...
public class ReportBedrockService {
    ...
    private String constructHitterPrompt(PredictionResult pred, String s3Context) {
        ...
        return String.format(
                "너는 KBO 리그 데이터 전략가이자 수석 스카우터입니다. 아래 데이터를 바탕으로 %s 선수의 '2026 시즌 프리뷰 리포트'를 작성하십시오.\n\n" +
                        ...
                        "[리포트 작성 및 포맷 지침 - 필독]\n" +
                        "1. **형식 엄수**: 모든 지표 명칭은 반드시 '영어(한글)' 형식을 유지하십시오. (예: MH(멀티히트), AVG(타율))\n" +
                        "2. **최상단 메인 서머리**: 리포트 시작 시 '[2026년 기술적 위상 요약]' 같은 제목을 절대 쓰지 마십시오. 곧바로 '## '을 사용하여 해당 선수의 2026년 핵심 가치를 관통하는 강렬한 요약 문장으로 시작하십시오.\n" +
                        "3. **소제목 완성**: 소제목의 따옴표(\"\") 안에는 가이드 문구가 아닌, 데이터를 분석해서 나온 '실제 통찰'이 들어가야 합니다. (예: ### 1. 2026 시즌 전체 전망: \"조정기 이후 0.9점대 OPS 안착이 기대되는 시즌\")\n" +
                        "4. **기술 용어 순화**: 'SHAP', '피처' 대신 '성적 결정 요인', '상승의 근거' 등으로 표현하고, 이모지는 절대 금지입니다.\n" +
                        "5. **강조 구문**: 핵심 수치와 결정적 결론은 ** 강조할 내용 ** 처럼 별표 앞뒤에 공백을 두어 표시하십시오.\n" +
                        "6. **문단 구성**:\n" +
                        "   - [최상단]: ## [선수명, 2026년 기술적 운명을 결정지을 한 줄 요약]\n" +
                        "   - ### 1. 2026 시즌 전체 전망: \"[데이터 기반의 전체적 성적 추세 요약]\"\n" +
                        "   - ### 2. 성장을 이끄는 기술적 동력: \"[SHAP 상승 요인 지표들이 만들어낼 긍정적 효과 요약]\"\n" +
                        "   - ### 3. 주의해야 할 변수와 리스크: \"[SHAP 하락 요인 지표들로 인해 경계해야 할 기술적 문제 요약]\"\n" +
                        "   - ### 4. 최종 기술 종합 평론: [지표들을 총망라하여 2026년의 전술적 위상을 심층 기술 (인사말/팬 멘트 금지)]\n\n" +
                        "- 말투: 냉철하고 전문적인 분석가용 경어체(~합니다)를 사용하십시오.",
