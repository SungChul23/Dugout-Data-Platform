    ...
    private String constructHitterPrompt(PredictionResult pred, String s3Context) {
        ...
        return String.format(
                "너는 KBO 리그 데이터 분석가이자 수석 스카우터입니다. 아래 제공된 SHAP 데이터를 기반으로 %s 선수의 '2026 시즌 프리뷰 리포트'를 작성하십시오.\n\n" +
                        ...
                        "2. **최상단 메인 서머리 (가장 크게)**: 리포트의 맨 처음에 '## 한 줄 요약 문구' 형식을 사용하여 가장 큰 글씨로 작성하십시오. 반드시 문구 앞에 '## '을 붙여야 합니다.\n" +
                        "3. **섹션 소제목 (중간 크기)**: 각 섹션 제목은 반드시 '### 숫자. 섹션명: \"데이터 요약 문구\"' 형식을 사용하십시오. 제목 앞의 '### '은 글씨 크기를 조절하는 마크다운 문법이므로 필수입니다.\n" +
                        "4. **데이터 중심 분석**: '부상 관리', '정신력' 같은 일반론은 배제하고, MH(멀티히트), BB%%, BABIP, GO/AO 등 제공된 수치만을 기술적으로 해석하십시오.\n" +
                        "5. **강조 구문**: 핵심 키워드는 ** 강조할 단어 ** 처럼 별표 앞뒤에 공백을 한 칸씩 두어 가독성을 높이십시오.\n" +
                        "6. **문단 구성**:\n" +
                        "   - [최상단]: ## 25년의 컨택 지표를 기반으로 26년 안정적 생산력을 유지할 핵심 타자, %s\n" +
