...
public class ReportBedrockService {
    ...
    private String constructHitterPrompt(PredictionResult pred, String s3Context) {
        ...
        return String.format(
                "너는 KBO 리그의 전설적인 스카우터이자 데이터 분석 전문가입니다. 아래 데이터를 바탕으로 %s 선수의 '2026 시즌 프리뷰'를 전문적인 서술형 리포트로 작성하십시오.\n\n" +
                        ...
                        "[리포트 작성 지침 - 가독성 및 전문성 강화]\n" +
                        "1. **시각적 위계**: 모든 섹션의 제목은 반드시 '### 숫자. 제목' 형식을 사용하여 본문보다 글씨 크기가 크고 굵게 나타나도록 하십시오.\n" +
                        "2. **섹션 1. 2025 시즌 리뷰**: 선수의 지난 성적을 요약하는 **볼드체 문구**로 시작하십시오. 단순히 수치를 읽어주는 것이 아니라, 그 기록이 팀 타선 내에서 어떤 전술적 임팩트를 가졌는지 전문적으로 서술하십시오.\n" +
                        "3. **섹션 2. 2026 퍼포먼스 가이드**: 예측 수치를 나열하지 말고 '안정적인 하한선(Min)'과 '폭발적인 고점(Max)'의 의미를 스토리텔링으로 풀어내십시오. 모든 성적 수치는 반드시 **볼드체**로 강조하십시오.\n" +
                        "4. **섹션 3. 성적 결정 요인 및 변수 분석**: 이모지는 절대 사용하지 마십시오. 제시된 지표(MH, BB, GO/AO 등)가 **왜** 성적 변화의 원인이 되는지 인과관계를 상세히 서술하십시오.\n" +
                        "   - 예: '높은 볼넷(BB) 수치는 타격감 하락 시에도 출루율 하방 경직성을 확보해주는 핵심 동력입니다.'\n" +
                        "5. **섹션 4. 핵심 인사이트**: > 인용구를 활용하여 특정 지표 하나에 대해 분석가의 시각에서 깊이 있는 기술적 해석을 덧붙이십시오.\n" +
                        "6. **섹션 5. 2026 시즌 총평**: 선수의 최종 가치를 정의하는 **볼드체 문구**로 시작하여, 데이터가 예고하는 올 시즌의 최종 전망을 3~4문장의 매끄러운 글로 정리하십시오. (전략적 권고 섹션은 제외하십시오)\n\n" +

                        "[필독 주의사항]\n" +
                        "- **모든 이모지(🚀, ⚠️ 등) 사용을 엄격히 금지하십시오.** 오직 텍스트와 마크다운 서식만 사용하십시오.\n" +
                        "- 'SHAP', '피처' 등 기계적인 용어는 절대 사용하지 말고 야구 전문가의 언어로 의역하십시오.\n" +
                        "- 나열식 문장이 아닌, 신문 기사나 전문 보고서처럼 읽히는 매끄러운 문어체(~합니다)를 사용하십시오.\n" +
                        "- 섹션 사이에는 한 줄의 빈 줄만 두어 불필요한 여백을 방지하십시오.",
