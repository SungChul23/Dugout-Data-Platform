    ...
    private String constructPitcherPrompt(PredictionResult pred, String s3Context) {
        ...
        return String.format(
                "너는 야구 데이터 분석 전문 '더그아웃'의 수석 스카우터야. %s 투수의 리포트를 작성해줘.\n\n" +
                        ...
                        "### [리포트 작성 규칙 - 수석 스카우터의 명령] ###\n" +
                        "1. **타이틀 형식**: 리포트의 첫 줄은 반드시 [2026 시즌 분석 리포트 - %s 선수] 로 시작할 것.\n" +
                        "2. **기호 사용 금지**: 마크다운 기호를 절대 사용하지 말고 텍스트와 줄바꿈으로만 구성할 것.\n" +
                        "3. **보직 중립성**: 특정 보직을 단정 짓지 말고 '마운드의 핵심 전력', '팀의 주축 자원' 등의 표현을 쓸 것.\n" +
                        "4. **지표의 신뢰도**: 현재의 성적이 내년에도 유지될 확률을 '성적의 지속 가능성' 측면에서 분석할 것.\n" +
                        "5. **에이징 커브 분석**: 데이터에 포함된 선수의 나이(age)를 반드시 확인하여 그에 맞는 분석을 제공할 것.\n" +
                        "   - 20대: 신체적 전성기 진입과 성장의 여지를 중심으로 서술.\n" +
                        "   - 30대 중반~40대: 풍부한 경험, 노련한 경기 운영, 철저한 자기관리와 베테랑의 가치를 중심으로 서술.\n" +
                        "6. **종합 평가**: 마지막 문단에는 팀의 마운드 뎁스와 승률 계산에 기여할 전략적 가치를 총평할 것.\n",
