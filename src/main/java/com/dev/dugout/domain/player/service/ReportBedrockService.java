    ...
    private String constructHitterPrompt(PredictionResult pred, String s3Context) {
        ...
        return String.format(
                "너는 KBO 리그 데이터 분석가이자 수석 스카우터입니다. 아래 제공된 SHAP 데이터를 기반으로 %s 선수의 '2026 시즌 프리뷰 리포트'를 작성하십시오.\n\n" +
                        ...
                        "[리포트 작성 및 포맷 지침 - 필독]\n" +
                        "1. **이모지/이모티콘 절대 금지**: 리포트 전체에서 이모지를 절대 사용하지 마십시오.\n" +
                        "2. **최상단 메인 서머리 (가장 크게)**: 리포트의 맨 처음에 '## '을 사용하여 2025년의 성과와 2026년의 기대를 관통하는 강렬한 한 줄 요약을 작성하십시오. (예: ## 25년의 정교함을 기반으로 26년 안정적 생산력을 유지할 핵심 타자, %s)\n" +
                        "3. **섹션 소제목 (중간 크기)**: 모든 소제목은 반드시 '### 숫자. 섹션명: \"데이터 요약 문구\"' 형식을 사용하십시오. 제목 앞의 '### '은 필수입니다.\n" +
                        "4. **강조 표시 활용**: 타율, 홈런, OPS 등 주요 예측 지표와 리포트의 핵심적인 분석 결론 문장은 반드시 ** 강조할 내용 ** 처럼 별표 앞뒤에 공백을 두어 강조하십시오.\n" +
                        "5. **데이터 중심 분석**: '부상 관리', '정신력' 같은 일반론은 배제하고, MH(멀티히트), BB%%, BABIP, GO/AO 등 제공된 수치만을 기술적으로 해석하십시오.\n" +
                        "6. **문단 구성**:\n" +
                        "   - [최상단]: ## 메인 한 줄 요약\n" +
                        "   - ### 1. 2026 전체 전망: \"조정기를 거쳐 안정적인 상위권 지표에 안착할 전망\"\n" +
                        "   - ### 2. 성장을 이끄는 핵심 동력: \"MH와 BB%% 수치에서 드러나는 견고한 타격 메커니즘\"\n" +
                        "   - ### 3. 경계해야 할 변수: \"BABIP 조정과 GO/AO 비율 최적화를 통한 효율성 개선\"\n" +
                        "   - ### 4. 최종 결론: \"데이터는 그의 전성기가 2026년에도 지속될 것임을 시사합니다\"\n\n" +
