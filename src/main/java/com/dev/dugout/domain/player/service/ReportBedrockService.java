...
public class ReportBedrockService {
    ...
    private String constructHitterPrompt(PredictionResult pred, String s3Context) {
        ...
        return String.format(
                "너는 KBO 리그의 전설적인 스카우터이자 데이터 분석 전문가입니다. 아래 데이터를 분석하여 %s 선수의 '2026 시즌 프리뷰'를 작성하십시오.\n\n" +
                        ...
                        "- 2026 예측 범위:\n" +
                        "  1) 타율: %.3f (최저 %.3f ~ 최고 %.3f)\n" +
                        "  2) 출루율: %.3f (최저 %.3f ~ 최고 %.3f)\n" +
                        "  3) 장타율: %.3f (최저 %.3f ~ 최고 %.3f)\n" +
                        "  4) OPS: %.3f (최저 %.3f ~ 최고 %.3f)\n" +
                        "  5) 홈런: %d개 (최저 %d ~ 최고 %d)\n" +
                        "- 분석 기반 데이터: \n%s\n\n" +

                        "[작성 규칙 - 가독성 최우선]\n" +
                        "1. **섹션 1 (2025 리뷰)**: '1. 2025 시즌 리뷰: **\"문구\"**'로 시작하십시오. 반드시 문구 전체를 **볼드체**로 감싸 선수에 대한 임팩트 있는 정의를 내리십시오.\n" +
                        "2. **섹션 2 (퍼포먼스 가이드)**: '2. 2026 퍼포먼스 가이드: **예측 범위**'를 소제목으로 하십시오. 모든 숫자와 성적 지표는 반드시 **볼드체**로 표기하십시오.\n" +
                        "3. **섹션 3 (동력/리스크)**: 소제목을 '3. 🚀 동력 및 ⚠️ 리스크 분석'으로 설정하여 이모지를 소제목에만 배치하십시오. 본문 내에서는 이모지 없이 **볼드체** 키워드만 사용하여 깔끔하게 서술하십시오.\n" +
                        "4. **섹션 4 (딥다이브)**: '4. 핵심 인사이트: **데이터의 이면**'을 소제목으로 하십시오. 인용구(`> `)와 **볼드체**를 활용해 기술적 지표(예: BABIP 등)를 알기 쉽게 설명하십시오.\n" +
                        "5. **섹션 5 (최종 총평)**: '2026 시즌 총평: **\"최종 요약 문구\"**'로 마무리하십시오. 문구 전체를 **볼드체**로 강조하십시오. (전략적 권고 섹션은 제외하십시오)\n\n" +

                        "[필독 주의사항]\n" +
                        "- **모든 핵심 수치와 소제목의 강조 문구는 반드시 볼드체(**...**)를 적용하십시오.**\n" +
                        "- 이모지는 소제목 외의 본문 문장 내에서는 사용하지 마십시오.\n" +
                        "- 'SHAP', '모델', '피처' 등 기계적인 용어는 절대 금지하며 야구 전문가의 언어로 의역하십시오.\n" +
                        "- 각 섹션은 3문장 정도로 간결하게 작성하여 전체 리포트가 끊기지 않도록 하십시오.\n" +
                        "- 전문가답고 열정적인 경어체(~합니다)를 유지하십시오." +
                        "- 섹션 사이에는 오직 한 줄의 빈 줄(Single empty line)만 허용하십시오.",
