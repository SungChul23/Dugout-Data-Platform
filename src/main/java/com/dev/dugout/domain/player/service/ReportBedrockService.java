    ...
    private String constructHitterPrompt(PredictionResult pred, String s3Context) {
        ...
        return String.format(
                "너는 KBO 리그의 전설적인 스카우터이자 데이터 분석가입니다. 아래 데이터를 바탕으로 %s 선수의 '2026 시즌 프리뷰 리포트'를 작성하십시오.\n\n" +
                        ...
                        "[리포트 작성 및 포맷 지침 - 필독]\n" +
                        "1. **최상단 임팩트 서머리**: 리포트의 맨 처음에 2025년의 실적과 2026년의 기대를 관통하는 '강렬한 한 줄 요약'을 작성하고 시작하십시오. (예: 25년의 침묵을 깨고 26년 불방망이를 휘두를 진격의 사자, OO 선수)\n" +
                        "2. **섹션 제목(Heading) 및 이모지**: 각 섹션의 제목은 반드시 마크다운의 '###'을 사용하여 글씨 크기를 차별화하고, 옆에 내용과 어울리는 이모지를 하나씩만 배치하십시오. 형식: ### 숫자. 섹션명: \"매력적인 문구\" 🎨\n" +
                        "3. **데이터의 스토리텔링**: 수치를 나열하지 마십시오. 'BABIP이 낮다'면 '불운을 뚫고 타격감을 유지했다'는 식으로 해석하십시오. 볼드체(**)는 핵심 키워드에만 정확하게 사용하십시오.\n" +
                        "4. **기술 용어 절대 금지**: 'SHAP', '피처' 대신 '상승의 원동력', '주의해야 할 변수' 등으로 표현하십시오.\n" +
                        "5. **문단 구성**:\n" +
                        "   - [최상단]: 한 줄 요약 (별도 제목 없이 강렬하게 시작)\n" +
                        "   - 1섹션: 2026 전체 전망 (전년 대비 변화와 예측 범위 강조)\n" +
                        "   - 2섹션: 성장을 이끄는 핵심 동력 (상승 요인 기반 분석)\n" +
                        "   - 3섹션: 주의해야 할 변수 (하락 요인 기반 분석)\n" +
