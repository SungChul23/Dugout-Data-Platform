...
| 기술 | 선택 이유 |
|------|-----------|
| **React (TypeScript / TSX)** | Google AI Studio로 UI 초안을 생성하고, 서버 API 연동·데이터 바인딩 로직은 직접 수정하여 완성. 컴포넌트는 TSX(TypeScript + JSX) 기반으로 구성되어 타입 안정성을 확보하면서 컴포넌트 단위로 UI를 독립적으로 관리 |
...
| 기술 | 선택 이유 |
|------|-----------|
| **EC2 (t3.micro)** | 서비스 서버 호스팅. 프리 티어 범위 내에서 운영 비용을 최소화하면서 실제 배포 환경을 경험하기 위해 선택 |
...
| **Amazon SageMaker** | ML 모델 학습 및 추론 환경. 로컬 환경의 리소스 제약 없이 안정적인 학습·추론 환경을 확보하기 위해 사용 |
...
| **Amazon SNS** | 골든글러브 모델 검증 이상 감지 시 즉시 알림 발송. 파이프라인 운영자에게 실시간으로 이슈를 전달 |
...
EventBridge (cron)
    └─▶ Step Functions 트리거
            ...
            └─▶ 정제 Lambda (수집 결과 통합 정제 → S3 저장)
                    ...
                    │       ├─ DB 적재 Lambda
                    │       └─ 모델 검증 Lambda (2026.05.07 추가) ← dugout-gg-validator
                    │
                    └─▶ [Branch B] DB 적재
...
> **2026.05.07 모니터링 강화**  
> 매일 추론 결과의 품질을 자동으로 검증하는 `dugout-gg-validator` Lambda를 파이프라인에 추가했습니다.  
> DB 적재 완료 후 S3의 타자·투수 리더보드를 읽어 포지션 누락, 예측 확률 이상, SHAP 기여도 결손 등  
> 총 10개 항목을 자동 점검합니다. 이상 감지 시 SNS를 통해 즉시 알림이 발송되며,  
> 검증 실패 여부와 무관하게 파이프라인 실행은 중단되지 않습니다.
...
### ✅ 왜 Google AI Studio로 프론트엔드를 구성했나?
UI 개발에 소요되는 시간을 최소화하고 **데이터 파이프라인·ML 모델링에 집중**하기 위해  
Google AI Studio로 컴포넌트 초안을 빠르게 생성했습니다.  
생성된 TSX(TypeScript + React) 코드에서 Spring 백엔드와의 API 연동, 데이터 바인딩,  
상태 관리 등 서비스 로직은 직접 수정하여 완성했습니다.  
AI 도구를 활용하되 **핵심 비즈니스 로직은 직접 구현**한다는 원칙을 지켰습니다.

### 🚨 AWS EC2 (t3.micro) 환경에서 OOM(Out Of Memory) 발생 및 Swap Memory를 통한 비용 최적화

* **문제 상황**
  * 서비스 운영 중 매일 오전 11시 데이터 적재 시점이 아닌, 새벽이나 오후 등 무작위 시점에 EC2 자바 프로세스가 강제 종료(`Killed`)되는 현상이 발생했습니다.
  * JVM 힙을 타이트하게 제한했음에도 실제 프로세스 점유 메모리(RSS)는 메타스페이스와 논힙 영역을 포함해 1GB인 물리 RAM 임계치에 근접했습니다. 11시 파이프라인 가동 시 유입된 대량 데이터 작업의 잔재가 메모리에 남아있다가, 한가한 시간대에 OS 백그라운드 루틴이 추가 메모리를 요구하는 순간 리눅스 커널의 OOM Killer에 의해 서버가 저격당한 것이 원인이었습니다.

* **해결 대안 및 의사결정**
  * **인스턴스 스펙 업그레이드 :** RAM을 2GB로 늘리면 즉시 해결되지만, 프리티어 범위를 벗어나 운영 비용이 2배 이상 증가하므로 프로젝트의 기조인 **비용 최적화 우선** 원칙에 위배되었습니다.
  * **디스크 기반 스왑 메모리(Swap Memory) 2GB 구축:** 하드디스크(SSD) 공간 일부를 가상 RAM으로 활용하는 완충 지대를 만드는 방향을 선택했습니다.

* **적용 결과 및 엔지니어링적 이점**
  * 리눅스 권장 가이드라인(물리 RAM의 2배)에 맞춰 **2GB의 스왑 파일(`swapfile`)을 생성 및 활성화**하고 자동 마운트 설정을 완료했습니다.
  * **메모리 로테이션 최적화:** 11시 피크 타임 시, 가득 찬 물리 RAM 대신 당장 연산하지 않는 백그라운드 프로세스들을 스왑 공간으로 격리(`Swap-out`)시키고, 작업 완료 후 여유가 생기면 다시 복귀(`Swap-in`)시키는 유기적 구조를 가집니다.
  * 결과적으로 추가 인프라 비용 지출 **0원**으로 서버 다운 장애를 완전히 해소했으며, 오버헤드를 최소화하면서 24시간 안정적인 배치 파이프라인 운영 환경을 확보했습니다.
