...
* **[타자] 공·수·기여 통합 평가**
    ...
    * **공격 (40%)**: OPS(80%) + RISP(20%) 기반.
    * **수비 (40%)**: FPCT(40%) + 수비 이닝 비중(60%) 기반.
    * **기여도 (20%)**: PA(40%) + 수비 이닝(40%) + 멀티 포지션 가산점(20%) 기반.

* **[투수] 구위·안정성·기여 통합 평가**
    * **공이 압도적인가(구위), 계산이 서는가(안정성), 많은 이닝을 소화하는가(기여도)** 를 핵심 가치로 설정.
    * **구위 (40%)**:ERA(40%) + WHIP(40%) + 피안타율(20%) 기반.
    * **안정성 이닝당 볼넷(70%) + 이닝당 피홈런(30%) 기반.
    * **기여도 소화 이닝(60%) + 홀드/세이브(30%) + 승률(10%) 기반.
